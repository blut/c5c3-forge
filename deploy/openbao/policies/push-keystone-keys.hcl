...
# credential-keys to a CR-scoped KV-v2 path. The original layout used a
# single CR-name segment — `openstack/keystone/{keystone.Name}/fernet-keys`
# (and …/credential-keys) — behind one `+` glob. As of the path is
# additionally scoped by namespace, shaped
# `openstack/keystone/{namespace}/{name}/fernet-keys` (namespace then name); the
# policy therefore uses a two-segment `+/+` glob, the added segment
...
# If this policy is later bound elsewhere (another cluster, a
...
# ESO's DeleteSecret for KV v2 issues DELETE on the data path
