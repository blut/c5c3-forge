...
- **Feedback**: Gateway path is used verbatim without basic validation or normalization (e.g. leading slash), which may lead to invalid HTTPRoute specs.
- **What was missed**: Path/URL fields from CR spec should be validated or normalized (e.g. prepend leading '/') before being used in downstream resources that will reject malformed input
- **Fix**: Gateway path normalization now prepends a leading slash when missing, with a new test covering the case
