...
### CC-0055 — berendt
- **Feedback**: If the imagetools inspect command fails or returns unexpected output, digest will be empty. The script writes digest= to GITHUB_OUTPUT without validation, and downstream composite steps will receive an empty digest.
- **What was missed**: After any variable assignment from a subshell or command substitution that feeds into GITHUB_OUTPUT, an environment variable, or a subsequent command argument, verify there is a non-empty check or set -o pipefail guard.
- **Fix**: Add a guard after the digest capture: `if [[ -z "$digest" ]]; then echo '::error::Failed to obtain digest'; exit 1; fi`
