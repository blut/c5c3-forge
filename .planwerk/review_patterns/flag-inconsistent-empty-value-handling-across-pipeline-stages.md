...
### CC-0017 — berendt
- **Feedback**: The Service always includes the webhook port (443→9443), but the Deployment conditionally omits the webhook container port when webhook.enabled=false [...] it's inconsistent with the conditional pattern applied everywhere else and will confuse operators debugging connectivity.
- **What was missed**: For every feature toggle in values.yaml, confirm that ALL templates referencing resources behind that toggle are wrapped in the same conditional. Cross-reference ports, volumes, volumeMounts, and Service targetPorts against Deployment container definitions.
- **Fix**: Wrapped the webhook Service port block in `{{- if .Values.webhook.enabled }}` to match the conditional pattern already used in the Deployment template.
