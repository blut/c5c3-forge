...
# quay.io/keycloak/keycloak image pin in the oidc-federation e2e fixture
# (and its chaos-suite sibling),
...
#
# Usage: bash tests/unit/renovate/keycloak_fixture_custommanager_test.sh
...
FIXTURE_FILE="$PROJECT_ROOT/tests/e2e/keystone/oidc-federation/00-keycloak.yaml"

KEYCLOAK_PACKAGE="quay.io/keycloak/keycloak"
FIXTURE_PATH="tests/e2e/keystone/oidc-federation/00-keycloak.yaml"

# --- Test 1: customManager targets the fixture and captures depName/tag/digest ---
test_custom_manager_captures_image() {
  echo "Test: customManagers regex captures the keycloak fixture depName/tag/digest"
  ...
  # The keycloak-fixture manager is the docker-datasource customManager on
  # the oidc-federation fixture file (shared with the chaos-suite sibling).
  local entry
  entry="$(jq -c '.customManagers[]
    | select(.datasourceTemplate == "docker")
    | select((.managerFilePatterns // []) | join(",") | contains("oidc-federation"))' \
    "$RENOVATE_FILE")"
  ...
  assert_contains "managerFilePatterns targets the fixture file" "$patterns" "oidc-federation"
  ...
  line="$(grep -E 'image: quay\.io/keycloak/keycloak' "$FIXTURE_FILE" | head -1)"
  match_string="$(jq -r '.matchStrings[0]' <<<"$entry")"
  assert_not_empty "keycloak image line present in the fixture" "$line"
  ...
  assert_contains "regex captures the depName" "$captured" "depName=${KEYCLOAK_PACKAGE}"
...
test_package_rules() {
  ...
  major_rule="$(jq -c --arg p "$KEYCLOAK_PACKAGE" --arg f "$FIXTURE_PATH" '.packageRules[]
  ...
  minor_rule="$(jq -c --arg p "$KEYCLOAK_PACKAGE" --arg f "$FIXTURE_PATH" '.packageRules[]
    | select(((.matchPackageNames // []) | index($p)) != null
             and ((.matchFileNames // []) | index($f)) != null
             and ((.matchUpdateTypes // []) | index("minor")) != null)' "$RENOVATE_FILE" | head -1)"

  if [ -z "$major_rule" ]; then
    echo "  FAIL: no packageRule scoping major updates for the keycloak fixture image"
    FAIL=$((FAIL + 1))
  else
    assert_eq "major keycloak fixture image updates are disabled" \
      "false" "$(jq -r '.enabled' <<<"$major_rule")"
  fi

  if [ -z "$minor_rule" ]; then
    echo "  FAIL: no packageRule scoping minor/patch updates for the keycloak fixture image"
    FAIL=$((FAIL + 3))
    return
  fi

  assert_eq "minor/patch keycloak fixture image updates are automerged" \
  ...
  assert_eq "minor/patch keycloak fixture image rule waits minimumReleaseAge=3 days" \
    "3 days" "$(jq -r '.minimumReleaseAge' <<<"$minor_rule")"
}

# --- Test 3: every keycloak fixture is Renovate-covered ---
# The keycloak pin is duplicated across e2e suites. A new suite that copies the
...
test_all_keycloak_fixtures_covered() {
  echo "Test: every keycloak fixture path is covered by the customManager and packageRules"
  ...
  local fixtures
  fixtures="$(cd "$PROJECT_ROOT" && grep -rl 'image: quay\.io/keycloak/keycloak' tests/ | sort)"
  assert_not_empty "at least one keycloak fixture exists" "$fixtures"

  local entry patterns f
  entry="$(jq -c '.customManagers[]
    | select((.managerFilePatterns // []) | join(",") | contains("keycloak"))' "$RENOVATE_FILE" | head -1)"
  ...
  while IFS= read -r f; do
    ...
    local rules
    rules="$(jq -r --arg p "$KEYCLOAK_PACKAGE" --arg f "$f" '[.packageRules[]
      | select(((.matchPackageNames // []) | index($p)) != null
