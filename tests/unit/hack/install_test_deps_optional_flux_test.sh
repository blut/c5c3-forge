...
test_default_skips_install_flux() {
  ...
  assert_not_contains "no 'flux ... already installed' log line" "$output" "flux 2.8.8 already installed"
  ...
  assert_contains "install_kubectl still runs" "$output" "kubectl v1.36.1 already installed"
...
test_with_flux_cli_true_invokes_install_flux() {
  ...
  assert_contains "install_flux was invoked" "$output" "Installing flux 2.8.8"
  ...
  assert_contains "install_flux completed" "$output" "flux 2.8.8 installed to"
...
test_with_flux_cli_true_short_circuits_on_correct_version() {
  ...
  assert_contains "short-circuit log line appears" "$output" "flux 2.8.8 already installed"
  # The download + verify branches must NOT have run.
  assert_not_contains "no 'Installing flux' log line" "$output" "Installing flux 2.8.8"
