...
# Verify docs/quick-start.md Step 3 coverage for CC-0088:
...
# --- Test 1: Step 2b row in the "What happens" table (CC-0088, REQ-008) ---
...
test_step_2b_table_row_present() {
  echo "Test: Step 3 'What happens' table contains a 2b row for Envoy Gateway + openstack-gw (CC-0088, REQ-008)"
...
test_step_2b_row_position() {
  echo "Test: Step 2b row is positioned between Step 2a and Step 3 in the table (CC-0088, REQ-008)"
...
# --- Test 3: Architecture snapshot lists envoy-gateway-system pod (CC-0088, REQ-008) ---
test_snapshot_lists_envoy_gateway() {
  echo "Test: 'What gets deployed' snapshot lists envoy-gateway-system  envoy-gateway-*  Ready (CC-0088, REQ-008)"
