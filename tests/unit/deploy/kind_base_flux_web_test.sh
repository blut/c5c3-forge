...
test_chart_version_pin_is_semver_range() {
  echo "Test: ResourceSet spec.inputs[0].version is 0.47.x and matches FLUX_OPERATOR_VERSION minor track (CC-0086, REQ-001)"
  ...
  if ! command -v yq >/dev/null 2>&1; then
    ...
    assert_eq "spec.inputs[0].version pins the 0.47.x SemVer range" "0.47.x" "$version"
  fi

  assert_file_contains "hack/deploy-infra.sh still declares FLUX_OPERATOR_VERSION=\"v0.47.0\"" \
    "$DEPLOY_INFRA_SH" \
